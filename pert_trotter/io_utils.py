    ...
    with h5py.File(file_str, "w") as f:
        for i, sparse_array in enumerate(array_list):
            ...
            group.create_dataset(
                "indices", data=sparse_array.indices, compression="gzip"
            )
            group.create_dataset("indptr", data=sparse_array.indptr, compression="gzip")
            group.attrs["shape"] = sparse_array.shape


def load_h5_list_of_sparse(
    file_str, index=None
):  # When index is provided, the only one sparse array at the index in the list is returned.
    """
    ...
    if index == None:
        with h5py.File(file_str, "r") as f:
            ...
            for i in range(len(f.keys())):
                ...
                array_list.append(
                    sp.sparse.csc_matrix((data, indices, indptr), shape=shape)
                )
            return array_list
    else:
        with h5py.File(file_str, "r") as f:
