...
def Do_LR(H_FO, shrink_frag, CISD, save = True):
  ...
  lowrank_fragments, lowrank_params = LR_frags_generator(tbt, tol = 1e-5, ret_params = True)
...
def Do_LR_LCU(H_FO, shrink_frag, CISD, save = True, load = True):
  ...
  if load == False:
    ...
    #Obtaining LR fragments as list of FermionOperators and (coeffs, angles) defining the fragments.
    LR_fragments_org, LR_params_org = LR_frags_generator(tbt, tol = 1e-5, ret_params = True)
  ...
  for temp_params in LR_params:
    ...
    term_1_spac = contract('ij,pi,qi,rj,sj->pqrs', coeff_mat, temp_params[1], temp_params[1], temp_params[1], temp_params[1])
    term_1_spin = spac2spin(term_1_spac)
    term_1_op = tbt2op(term_1_spin)

    term_2_spac = contract('ij,pi,qi->pq', coeff_mat/2, temp_params[1], temp_params[1])
    term_2_spin = 2*spac2spin(term_2_spac)   #Factor of 2 appears due to the spin degree of freedom.
    term_2_op = obt2op(term_2_spin)

    term_3_spin = 4*np.sum(coeff_mat)/4     #Factor of 4 appears due to the spin degree of freedom. We are assuming LR is done in spacial orbital basis.
...
def Do_SD_GFRO(H_FO, shrink_frag, CISD, tol=1e-6, save = True, spacial = False):
  ...
  gfro_fragments, gfro_params_1, gfro_params_2 = sdgfro_frags_generator(obt, tbt, tol = tol, ret_params=True, spacial = False)
...
def Do_GFRO_LCU(H_FO, shrink_frag, CISD, tol=1e-6, save = True, load = True, spacial = False):
  ...
  for temp_params, frag in zip(gfro_params, gfro_fragments):
    ...
    term_3_spin = 4*np.sum(coeff_mat)/4     #Factor of 4 appears due to introducing spin degree of freedom. We are assuming GFRO is done in spacial orbital basis.
