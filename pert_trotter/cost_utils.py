...
def countOneRotFerm(Nqubs, NFrags, ord = 1):
    ...
    if ord ==1:
      ...
      dum = Nqubs**2 * (4*NFrags - 3) - Nqubs         #This follows from the fact that 2nd order Trotter has 2Nfrags -1 exponentials and of them, 2 are one-body operators.
    return dum
...
def optimize_T_gates(x0, eps, Tot_Err, Nr, scale, use_constraints = True):
    ...
    if use_constraints == True:
      ...
      result = minimize(count_T_gates, x0, args=(eps, Tot_Err, Nr, scale), bounds=bounds, constraints=constraints, method='cobyla', options={'maxiter': 1000})
      ...
      x0_new = [x0[1], x0[2]]
      # Perform minimization
      result = minimize(count_T_gates, x0_new, args=(eps, Tot_Err, Nr, scale), bounds = bounds)
  ...
  for i in range (denom):
    for j in range (denom):
      for k in range (denom):
        if i+j+k+3 < denom:
          x0 = [(i+1)*Tot_Err/denom, (j+1)*Tot_Err/denom, (k+1)*Tot_Err/denom]
          optimal_errs, min_Nt = optimize_T_gates(x0, err_est, Tot_Err, Nr, scale, use_constraints=use_constraints)
