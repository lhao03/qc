...
def spac2spin(tensor):
  ...
  if len(tensor.shape) == 4:
    ...
    idx_uudd = np.ix_(*[range(0,2*N,2), range(0,2*N,2), range(1,2*N,2), range(1,2*N,2)])
    idx_dduu = np.ix_(*[range(1,2*N,2), range(1,2*N,2), range(0,2*N,2), range(0,2*N,2)])
    spin_tbt[idx_uuuu] = spin_tbt[idx_dddd] = spin_tbt[idx_uudd] = spin_tbt[idx_dduu] = tensor
    return spin_tbt
...
def obt2tbt(obt):
  ...
  """
  spac_obt = obt[np.ix_(*[range(0,i,2) for i in obt.shape])]    #Working with spacial orbitals: Extracting only one of the spin flavours
  N = spac_obt.shape[0]
...
def Bliss_K_tensors(ovec, t1, t2, η, n_qubits):    #(n_qubits = n_qubits, len(ovec) = 2*(n_qubits/2)*(n_qubits/2 + 1)/2)
  '''
          ...
          for s in range (N):
            for a in range(N):
              for b in range(N):
                for c in range (N):
                  for d in range (N):
                    term = ((p,1), (q,0),(r,1), (s,0), (a,1), (b,0),(c,1), (d,0))
...
def get_chem_tensors_hubbard(H_chem, N = None):        #Does not work in general.
  ...
  for key, value in H_chem.terms.items():
    if len(key) == 4:
      ...
      if np.abs(p-q)%2 != 0:         #If p-q is not even, it means p and q are of different spin flavour
        q, s = s, q                  #Interchange q with s so that p&s and r&q are now of the same spin flavour.
        value = -value
  ...
  for key, value in H.terms.items():
    if len(key) == 4:
      if key[1][1] == 1:                                                      #Checking if the given term is physics ordered.
        ...
        for key, value in H.terms.items():
          if len(key) == 4:
            chem_tbt[key[0][0], key[1][0], key[2][0],key[3][0]] += value/2
            chem_tbt[key[2][0], key[3][0], key[0][0],key[1][0]] += value/2
          elif len(key) == 2:
